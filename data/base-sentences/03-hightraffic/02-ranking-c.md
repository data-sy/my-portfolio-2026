...
| question_1 | 매 요청마다 전체 테이블 COUNT + ORDER BY 정렬하면 부하가 얼마나 될까? |
| question_2 | 캐싱하면 해결될까? 랭킹 갱신은 어떻게? |
| question_3 | 랭킹에 최적화된 자료구조가 있을까? |
| question_4 | null |
| question_5 | null |
| try_1_title | DB 인덱스 추가 |
| try_1_desc | sales_count 컬럼에 인덱스 생성하여 정렬 최적화 |
| try_1_result | 200ms → 150ms (25%↓) |
| try_1_limit | 매 요청마다 정렬 비용 발생, 동시 요청 시 DB 부하 |
| try_2_title | Redis String 캐싱 |
| try_2_desc | 랭킹 결과를 Redis에 캐싱, TTL 만료 시 DB 재조회 |
| try_2_result | 200ms → 20ms (90%↓) |
| try_2_limit | 갱신 시 전체 재계산 필요, race condition 가능성 |
| try_3_title | Redis Sorted Set |
| try_3_desc | ZINCRBY로 점수 갱신, ZREVRANGE로 TOP N 조회. 정렬 상태 자동 유지 |
