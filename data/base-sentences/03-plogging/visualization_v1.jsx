        ...
        <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`}
          ...
          {showBetween && (
            <rect x={cx - sqHalf * scale} y={cy - sqHalf * scale}
              ...
              fill={COLORS.warnDim} stroke={COLORS.warn} strokeWidth={1.5} strokeDasharray="6 3"
              opacity={activeMethod === "between" ? 0.9 : 0.4} />
          )}
            ...
            <div style={{
              ...
              <div style={{ fontFamily: "monospace", fontSize: 12, color: COLORS.text, lineHeight: 1.8 }}>
                ...
                <div style={{ marginTop: 6, paddingTop: 6, borderTop: `1px solid ${COLORS.cardBorder}` }}>
                  평균 오차: <span style={{ color: COLORS.warn }}>135m (13.5%)</span>
                </div>
              </div>
            </div>
          )}
          ...
          {(activeMethod === "all" || activeMethod === "haversine") && (
            <div style={{
              ...
              <div style={{ fontFamily: "monospace", fontSize: 12, color: COLORS.text, lineHeight: 1.8 }}>
                ...
                <div>평균 오차: <span style={{ color: COLORS.success }}>1.2m (0.12%)</span></div>
                <div style={{ marginTop: 6, paddingTop: 6, borderTop: `1px solid ${COLORS.cardBorder}` }}>
                  개선율: <span style={{ color: COLORS.accent }}>135m → 1.2m (99.1% 개선)</span>
                </div>
              </div>
            </div>
          )}
    ...
    <div>
      <div style={{
        ...
        <h4 style={{ color: COLORS.text, margin: "0 0 8px", fontSize: 14 }}>
          오차율 13.5%는 어떻게 나온 숫자?
        </h4>
        <p style={{ color: COLORS.textDim, fontSize: 13, margin: 0, lineHeight: 1.7 }}>
          8방향에서 측정한 유클리드 오차율의 <strong style={{ color: COLORS.accent }}>평균</strong>이에요.
          N/S는 거의 정확(0.3%)하지만 E/W는 26%나 되고, 이걸 평균 내면 13.5%가 됩니다.
        </p>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", background: COLORS.card, borderRadius: 12 }}>
          ...
          <tbody>
            ...
            <tr style={{ background: "#1e2030", borderTop: `2px solid ${COLORS.accent}` }}>
              <td style={{ ...cellStyle, textAlign: "center", color: COLORS.accent, fontWeight: 700 }}>평균</td>
              <td style={{ ...cellStyle, textAlign: "right", color: COLORS.textDim }}>1000.0m</td>
              <td style={cellStyle}></td>
              <td style={{ ...cellStyle, textAlign: "right", color: COLORS.warn, fontWeight: 700 }}>135.2m</td>
              <td style={{ ...cellStyle, textAlign: "right", color: COLORS.warn, fontWeight: 700 }}>13.52%</td>
              <td style={cellStyle}></td>
              <td style={{ ...cellStyle, textAlign: "right", color: COLORS.success, fontWeight: 700 }}>1.18m</td>
              <td style={{ ...cellStyle, textAlign: "right", color: COLORS.success, fontWeight: 700 }}>0.12%</td>
            </tr>
          </tbody>
        </table>
      </div>
