...
| situation | View에 상태 관리 로직이 섞여 있고, TimerManager가 데이터 저장·비즈니스 로직·완료 처리를 모두 담당. 정책 변경(완료 시 보관→자동삭제) 시 수정 범위가 넓고 사이드이펙트 파악이 어려움 |
...
| try_3_limit | null |
| final_result | SRP 기반 계층 분리로 정책 변경 시 수정 대상 명확화. 데이터 수정은 Repository, 비즈니스 로직은 Service만 통과하는 SSOT 확보 |
