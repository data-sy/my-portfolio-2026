...
followup 답변:
| followup_a1 | JavaScript(Node.js)를 쓴다면 TypeScript 도입이 첫 번째 선택. 컴파일 타임에 타입 체크를 추가해서 Swift처럼 안전성을 앞당기는 효과. 다만 TypeScript도 런타임에는 타입이 사라지므로 Zod 같은 런타임 검증 라이브러리를 병행해야 완전한 안전성 확보. 결국 언어가 보장 안 하는 부분은 도구로 보완하는 것이고, Swift→Java→JavaScript 순으로 "개발자가 직접 챙겨야 할 안전성"이 늘어나는 구조 |
| followup_a2 | "이 언어는 안전성을 언제 보장하는가?"를 가장 먼저 봄. 컴파일 타임 강제가 많으면 초기 학습 비용은 높지만 런타임 버그가 줄고, 런타임 유연성이 높으면 빠르게 프로토타이핑 가능하지만 방어 코드가 늘어남. Swift→Java→JavaScript 경험으로 이 트레이드오프를 체감했기 때문에, 새 언어를 볼 때 Null 처리·타입 시스템·메모리 모델을 기준으로 스펙트럼 어디쯤인지 먼저 파악하고 학습 전략을 세움 |
-->
