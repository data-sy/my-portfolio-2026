...
| target_a | DB에서 Haversine 계산 |
| target_b | 애플리케이션(Java)에서 Haversine 계산 |
| criteria_1 | DB CPU 부하 |
| cri_1_a | 모든 row에 삼각함수 연산 필요. DB CPU 부하 |
| cri_1_b | DB는 단순 조회만 수행. 삼각함수 연산은 Java에서 처리 |
| criteria_2 | 네트워크 전송량 |
| cri_2_a | 필요한 결과만 반환. 전송량 최소화 |
| cri_2_b | 전체 데이터를 DB에서 애플리케이션으로 전송. 네트워크·메모리 부담 |
| criteria_3 | 삼각함수 정밀도 |
| cri_3_a | DB 내장 수학 함수 사용 |
| cri_3_b | Java Math 라이브러리의 정밀한 삼각함수 활용 |
| criteria_4 | 인덱스 활용 |
| cri_4_a | WHERE절에 함수 사용 시 인덱스 활용 불가 |
| cri_4_b | DB는 BETWEEN으로 인덱스 활용 가능. 함수 연산은 Java에서 분리 |
| criteria_5 | null |
| cri_5_a | null |
| cri_5_b | null |
| result | 하이브리드: DB에서 BETWEEN으로 1차 필터링 + Java에서 Haversine으로 2차 필터링 |
| result_desc | BETWEEN의 사각형이 Haversine의 원을 포함하므로 1차 필터링에서 누락 없음. DB는 인덱스 활용 가능한 단순 범위 조회만 수행하고, 삼각함수 연산은 축소된 후보군에 대해서만 Java에서 실행. 정확도 손해 없이 양쪽의 단점을 제거 |
